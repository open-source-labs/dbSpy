...
// Validation Constants (collapse these, store is below)
const restrictedPgNames = {
  ALL: true,
  ANALYSE: true,
  ANALYZE: true,
  AND: true,
      ...
      (set, get) => ({
        ...
        addColumnSchema: (tableName, columnDataArr) =>
          set((state) => {
            ...
            const currentTable = newState.schemaStore[tableName];
            for (const columnData of columnDataArr) {
              const newCol: ColumnSchema = {
                Name: columnData.name,
                Value: columnData.defaultValue,
              ...
              currentTable[columnData.name] = newCol;
            }
