...
const useSchemaStore = (0, zustand_1.default)()(
...
(set, get) => ({
    ...
    addTableSchema: (tableName, columnDataArr) => set((state) => {
        ...
        newState.schemaStore = get()._addColumns(newState.schemaStore, tableName, columnDataArr);
        get()._addHistory(newState);
        return newState;
    }),
    deleteTableSchema: (tableName) => set((state) => {
        const newState = { ...state };
        delete newState.schemaStore[tableName];
        get()._addHistory(newState);
        return newState;
    }),
    addColumnSchema: (tableName, columnDataArr) => set((state) => {
    ...
    addForeignKeySchema(referenceData) {
        set((state) => {
            ...
            const destinationTable = referenceData.PrimaryKeyTableName;
            const destinationColumn = referenceData.PrimaryKeyName;
            const newState = {
                ...state,
                schemaStore: {
                    ...state.schemaStore,
    ...
    //---------------------------------
    deleteColumnSchema: (tableRef, columnRef) => set((state) => {
    ...
    redoHandler: () => {
        set((state) => {
            const newState = { ...state };
            if (newState.historyCounter >= newState.history.length - 1)
                return newState;
            newState.historyCounter += 1;
            newState.schemaStore = newState.history[newState.historyCounter];
            return newState;
    ...
    _checkTableValidity(tableName, columnDataArr) {
        ...
        if (columnDataArr) {
            ...
            if (pkCount !== 1)
                throw new Error(`Table must have one primary key (currently has ${pkCount})`);
    ...
    _checkColumnValidity(tableName, columnDataArr) {
        ...
        for (const column of columnDataArr) {
            ...
            if (Object.hasOwn(currentTable, column.name))
                throw new Error(`Table "${tableName}" already contains column named "${column.name}"`);
        }
        let existingPks = 0;
        for (const colKey in currentTable) {
            if (currentTable[colKey].IsPrimaryKey)
                existingPks++;
        }
        const newPks = get()._countPrimaries(columnDataArr);
        if (existingPks + newPks !== 1)
            throw new Error(`Table must have one primary key (currently has ${existingPks + newPks})`);
    ...
    _checkColumnNamesAndDupes(columnDataArr) {
        ...
        for (const { name } of columnDataArr) {
            ...
            if (nameRegister[name])
                throw new Error(`Table must not contain duplicate column names (cause: "${name}")`);
            else
                nameRegister[name] = true;
        }
    },
    _checkNameValidity(name) {
        const system = get().system;
        const restrictedNames = system === 'PostgreSQL'
        ...
        if (Object.hasOwn(restrictedNames, name.toUpperCase()))
            throw new Error(`Table and column names must not be ${system} syntax (cause: "${name}")`);
        if (!name.match(/^[a-zA-Z0-9_]*$/))
            throw new Error(`Name must only contain letters, numbers, and underscores (cause: "${name}")`);
    },
...
exports.default = useSchemaStore;
