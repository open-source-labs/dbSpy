...
export default function Canvas({
  ...
  */
  ...
  const mouseOverHandler = (event: React.MouseEvent<HTMLDivElement>) => {
    const eachTable = event.currentTarget;
    eachTable.style.backgroundColor = 'orange';
  }
  
  const mouseOutHandler = (event: React.MouseEvent<HTMLDivElement>) => {
    const eachTable = event.currentTarget;
    eachTable.style.backgroundColor = 'red';
  }
  ...
  const mouseOverTable = async () => {
    await document.getElementById('public.films')?.style.backgroundColor;
    ('pink');
    ...
    //   //grab that tables id
    //   const eachTableId = tables[i].props.id;
    //   // console.log('THIS IS THE TABLE ID FROM INSIDE LOOP', tables[0].props);
    //   //grab the ids of the document
    //   const documentId = document.getElementById(`${eachTableId}`);
    //   console.log(
    //     'THIS IS DOCUMENTIDs', documentId
    //   );
    //   //if the documentId matches the tableId
  ...
  window.addEventListener('DOMContentLoaded', async () => {
    for (let i = 0; i < tables.length; i += 1) {
      const eachTableId = await tables[i].props.id;
      // const documentId = await document.getElementById(`${eachTableId}`);
      const filmsTable = await document.getElementById('public.films');

      //     const mouseOverTable = async (): Promise<void> => {
      //       await document.getElementById('public.films')?.style.color; 'pink';
      //     }
      //     const mouseOut = async() => {
      //   await document.getElementById('public.films')?.style.color; 'red';
      // }
      filmsTable?.addEventListener('mouseover', mouseOverTable);
      filmsTable?.addEventListener('mouseout', mouseOut);
    }
  });
  ...
  const renderArrowsEvent = () => {
    for (let i = 0; i < tables.length; i += 1) {
      ...
      const mouseOut = async () => {
        await document.getElementById('public.films')?.style.color;
        ('red');
      };
      console.log('THIS IS EACHTABLEID', eachTableId);
      console.log('THIS IS DOCUMENTID', documentId);
      //
      // const eachArrow = document.querySelector('#public.films');

      // console.log('THIS IS QUERY SELECTOR PUBLIC.FILMS', eachArrow)

      // const draggableTables = document.querySelector('.table-DraggableSub2')

      // console.log('THIS IS DRAGGABLETABLES', draggableTables)

      // documentId.addEventListener('mouseover', mouseOverTable);

      // console.log(
      //   'THIS IS INSIDE RENDERARROWS',
      //   tables[i].key,
      //   tables[i].props
      // );
    }
...
/**
 * The function takes in an event, and then sets the background color of the currentTarget to pink
 * @param event - React.MouseEvent<HTMLDivElement>
 */
// const mouseOverHandler = (event: React.MouseEvent<HTMLDivElement>) => {
//   const eachTable = event.currentTarget;
//   eachTable.style.backgroundColor = 'pink';
// }
...
// }
  ...
  return (
    // style={{ height: "100%"}}
    <div ref={reference}>
      <button id="testing" onClick={mouseOverTable}>
        mouseOver
      </button>
      <button id="test" onClick={renderArrowsEvent}>
        renderArrows
      </button>
